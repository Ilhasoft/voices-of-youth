                ...
                gc.geocode({
                    ...
                    if (results && results[0]) {
                        ...
                        $.each(results[0].address_components, function(i, item) {
                            ...
                            // prefixo
                            if (results[0].address_components[i]['types'] == 'premise')
                                prefixo = results[0].address_components[i]['long_name'];
                            ...
                            if (results[0].address_components[i]['types'] == 'route') {
                                //console.log(results[0].address_components[i]['long_name']);
                                rua = results[0].address_components[i]['long_name'];
                            }
                            ...
                            // cidade
                            if (results[0].address_components[i]['types'][0] == 'locality' || results[0].address_components[i]['types'][0] == 'administrative_area_level_2') {
                                //console.log(results[0].address_components[i]['long_name']);
                                $('#id_city option')
                                    .filter(function(index) { return $(this).text() === results[0].address_components[i]['long_name']; })
                                    .prop('selected', true);
                            }
                        });
                        is_edit = false;
                        $('#id_address').val((prefixo ? prefixo : '') + (rua ? (prefixo ? ' - ' + rua : rua) : '') + (bairro ? ', ' + bairro : ''));
                    }
                });
            }
